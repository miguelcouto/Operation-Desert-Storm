...
if (isServer) then {diag_log "Krose RPT: Init - executing MAD_civilians.sqf"}; // Reporting. Do NOT edit/remove
...
_civKia_script = _this select 3; // CivKia EH
	...
	if (count _temphouses > 0) then {
		for "_b" from 1 to _count do {
			...
			if (krose_enableDebug) then {
				["Civilian Spawn", format["Foi spawnado %1 civis (%2) na posição %3", _class, _civ, _spawnpos]] call krose_fnc_hint;
			};
			...
			_civ setVariable ["MAD_civ_owner", "server"];
			_civ setspeedmode "LIMITED";
			_civ setBehaviour "SAFE";

			if (isServer && _civKia_script) then {_civ addEventhandler ["killed", krose_fnc_checkCivKia];};
