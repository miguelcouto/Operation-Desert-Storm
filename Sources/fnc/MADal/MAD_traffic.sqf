...
if (isServer) then {diag_log "Krose RPT: Init - executing MAD_traffic.sqf"}; // Reporting. Do NOT edit/remove
...
MAD_spawnCar = {
	...
	if (count _roads > 0) then {
		...
		if (krose_enableDebug) then {
			["Civilian Spawn", format["Foi spawnado %1 veículo (%2) na posição %3", _driver, _civ, _spawnpos]] call krose_fnc_hint;
		};

		if (isServer && _civKia_script) then {_civ addEventhandler ["killed", krose_fnc_checkCivKia];};	
		[_civ] call MAD_carWaypoint;
	};
};
